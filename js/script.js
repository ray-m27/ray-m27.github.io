$(document).ready(function(){
    /* $('.carousel__inner').slick({
        ...
        //адаптация под мобильные устр. 992 - точка перехода
        responsive: [
            {
                breakpoint: 992,
                settings: { //замена стрелок на точки
                    dots: true,  
                    arrows: false
                }
            }
    ...
    //отправление данных на почту front
    $('form').submit(function(e) {
      // e - event preventDefault() - отменить стандартное поведение браузера
        e.preventDefault(); 
        if(!$(this).valid()) {
          return;
        }
        ...
        $.ajax({
          ...
          $(this).find("input").val("");  //обнуляем
          $('#consultation, #order').fadeOut();
          $('.overlay, #thanks').fadeIn('slow');
          $('form').trigger('reset'); //сброс
        });
        return false; 
    });
